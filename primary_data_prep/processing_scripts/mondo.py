...
# Keep only true MONDO terms using raw CURIE from parser.
# NOTE:
# - rec.raw_item_id stores original value from OBO, e.g. "MONDO:0000001", "BFO:0000001".
# - rec.item_id keeps backward-compatible normalized value.
mondo_data = [
    x for x in data
    if isinstance(getattr(x, "raw_item_id", ""), str)
    and getattr(x, "raw_item_id", "").startswith("MONDO:")
]
...
mondo_terms = pd.DataFrame([{'id':x.item_id,
                             ...
                             'replacement_id': x.replaced_by} for x in mondo_data])
...
for x in mondo_data:
    if x._parents: 
        for parent in x._parents: 
            # MONDO parents are normalized numeric tails (e.g. "1234").
            # Cross-ontology parents are CURIEs containing ":".
            if ":" in str(parent):
                continue
