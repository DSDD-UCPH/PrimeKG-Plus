...
class OBOReader(object):
    ...
    def _add_to_obj(self, rec_curr, typedef_curr, line):
        ...
        if rec_curr is not None:
            ...
            # Some OBO files may repeat typedef name lines.
            # goatools.typedef.add_to_typedef asserts on duplicate `name:`.
            # Skip repeated typedef names to keep parser robust.
            if line[:6] == "name: " and getattr(typedef_curr, "name", None):
                return
    ...
    def _add_to_ref(self, rec_curr, line):        
        ...
        if line[:4] == "id: ":
            ...
            raw_item_id = line[4:]
            # Preserve full CURIE so callers can reliably filter source ontology
            # (e.g., MONDO:, BFO:, CHEBI:).
            rec_curr.raw_item_id = raw_item_id
            # Backward compatibility: old PrimeKG code expects MONDO numeric tail
            # in item_id/id when CURIE is MONDO:xxxxxxx.
            if raw_item_id.startswith("MONDO:"):
                item_id = raw_item_id[l:]
            else:
                item_id = raw_item_id
            ...
            raw_alt = line[8:]
            if raw_alt.startswith("MONDO:"):
                rec_curr.alt_ids.add(raw_alt[l:])
            else:
                rec_curr.alt_ids.add(raw_alt)
        elif line[:6] == "name: ":
            # Some MONDO stanzas can surface repeated name lines via imports.
            # Do not crash parsing; keep the first non-empty name.
            if not rec_curr.name:
        ...
        #    rec_curr.namespace = line[11:]
            ...
            raw_parent = line[6:].split()[0]
            if raw_parent.startswith("MONDO:"):
                rec_curr._parents.add(raw_parent[l:])
            else:
                rec_curr._parents.add(raw_parent)
        elif line[:8] == "subset: ": 
       ...
       #     rec_curr.synonyms.add((name,rel))            
            ...
            raw_repl = line[13:]
            if raw_repl.startswith("MONDO:"):
                rec_curr.replaced_by = raw_repl[l:]
            else:
                rec_curr.replaced_by = raw_repl
...
class GOTerm(object):
    ...
    def __init__(self):
        ...
        self.raw_item_id = ""       # Original CURIE from OBO, e.g. MONDO:0000001
