...
Curated input pool (shipped under ``dataset/PrimeKG-Plus-RD/curated/``):

  - ``*_final.csv`` — algorithm-mapped relations per disease
  - ``*_additional.csv`` — expert-merge additional relations

Outputs (under ``dataset/PrimeKG-Plus-RD/`` by default)
-----------------------------------------------------------
...
Usage: ``python 09_integrate_primekg_plus_rd.py``  |  ``python 09_integrate_primekg_plus_rd.py --dry-run``
       ``python 09_integrate_primekg_plus_rd.py --review-only``  — write resolved relation CSVs only (no primekg_plus_rd)

Step **09** in the literature-curation audit trail (see ``README.md`` in this folder).
``integrate_primekg_plus_rd.py`` is a backward-compatible alias for this script.
"""
...
def resolve_primekg_root() -> Path:
    ...
    release = resolve_release_root()
    for candidate in (release.parent, release.parent.parent):
        if (candidate / "datasets").is_dir():
            return candidate
    raise FileNotFoundError(
        "Set PRIMEKG_ROOT to the PrimeKG repo root (directory containing datasets/)."
    )
...
def default_disease_specs(curated_dir: Path) -> list[DiseaseSpec]:
    return [
        DiseaseSpec("Canavan", curated_dir / "Canavan_final.csv", curated_dir / "Canavan_additional.csv"),
        DiseaseSpec("Batten", curated_dir / "Batten_final.csv", curated_dir / "Batten_additional.csv"),
        DiseaseSpec("NPC", curated_dir / "NPC_final.csv", curated_dir / "NPC_additional.csv"),
        DiseaseSpec("Tay-Sachs", curated_dir / "Tay-Sachs_final.csv", curated_dir / "Tay-Sachs_additional.csv"),
    ]
...
def collect_literature_rows(specs: list[DiseaseSpec]) -> pd.DataFrame:
    ...
    if not frames:
        raise FileNotFoundError("No literature CSV files found under dataset/PrimeKG-Plus-RD/curated/.")
...
def main() -> None:
    ...
    curated_dir = Path(
        os.environ.get(
            "CURATED_DIR",
            release_root / "dataset" / "PrimeKG-Plus-RD" / "curated",
        )
    )
    data_dir = Path(
        os.environ.get("PRIMARY_DATA_DIR", release_root / "primary_data_prep" / "data")
    )

    plus_kg = Path(os.environ.get("PLUS_KG", release_root / "dataset" / "PrimeKG-Plus" / "primekg_plus.csv"))
    plus_nodes = Path(os.environ.get("PLUS_NODES", release_root / "dataset" / "PrimeKG-Plus" / "nodes.csv"))
    out_dir = Path(os.environ.get("KG_RD_OUT_DIR", release_root / "dataset" / "PrimeKG-Plus-RD"))
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"PrimeKG root:    {primekg_root}")
    print(f"Base kg:         {plus_kg}")
    print(f"Base nodes:      {plus_nodes}")
    print(f"Curation root:   {curated_dir}")
    ...
    specs = default_disease_specs(curated_dir)
    ...
    provenance_path = out_dir / "literature_edges_integrated.csv"
    skipped_path = out_dir / "literature_edges_skipped.csv"
    novel_path = out_dir / "literature_edges_novel.csv"
    nodes_added_path = out_dir / "literature_nodes_added.csv"
    summary_path = out_dir / "primekg_plus_rd_integration_summary.json"
    ...
    kg_rd_path = out_dir / "primekg_plus_rd.csv"
    nodes_rd_path = out_dir / "primekg_plus_rd_nodes.csv"
    edges_rd_path = out_dir / "primekg_plus_rd_edges.csv"
