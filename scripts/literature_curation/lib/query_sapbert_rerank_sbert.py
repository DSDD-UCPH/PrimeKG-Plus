...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory with embedding_manifest.json and memmaps (default: $PRIMEKG_ROOT/datasets/data/umls/embedding_pool_2025AB)",
    )
    p.add_argument(
        "--queries",
        type=str,
...
def resolve_out_dir(arg: Path | None) -> Path:
    ...
    if env := os.environ.get("UMLS_EMBED_DIR"):
        return Path(env).expanduser().resolve()
    if env := os.environ.get("PRIMEKG_ROOT"):
        return Path(env).expanduser().resolve() / "datasets/data/umls/embedding_pool_2025AB"
    raise FileNotFoundError(
        "Pass --out-dir or set UMLS_EMBED_DIR (or PRIMEKG_ROOT pointing at the PrimeKG repo)."
    )
